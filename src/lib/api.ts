...
export const endpoints = {
  ...
  livekitToken: (roomId: string, signal?: AbortSignal) => api<LiveKitConnection>(`/rooms/${encodeURIComponent(roomId)}/livekit-token`, { method: "POST", body: "{}", signal }),
  guestLivekitToken: (roomId: string, memberId: string, signal?: AbortSignal) => api<LiveKitConnection>(`/rooms/${encodeURIComponent(roomId)}/members/${encodeURIComponent(memberId)}/livekit-token`, { method: "POST", body: "{}", signal }),
  ...
  songs: (search = "") => api<import("../types").Song[]>(`/songs${search ? `?search=${encodeURIComponent(search)}` : ""}`),
  createSong: (song: { title: string; artist: string; defaultKey: string; bpm: number; sections: { sectionLabel: string; lyrics: string; displayOrder: number }[] }) => api<import("../types").Song>("/songs", { method: "POST", body: JSON.stringify(song) }),
  updateSong: (song: { id: string; title: string; artist: string; defaultKey: string; bpm: number; sections: { id?: string; sectionLabel: string; lyrics: string; displayOrder: number }[] }) => api<import("../types").Song>(`/songs/${song.id}`, { method: "PUT", body: JSON.stringify(song) }),
  deleteSong: (id: string) => api(`/songs/${id}`, { method: "DELETE" }),
  setRoomSongState: (roomId: string, songId?: string, songSectionId?: string) => api<import("../types").WorshipRoom>(`/rooms/${roomId}/song-state`, { method: "PUT", body: JSON.stringify({ songId: songId || null, songSectionId: songSectionId || null }) }),
