...
const section = (order = 1): SongSection => ({
  id: "",
  songId: "",
  sectionLabel: order === 1 ? "Verse 1" : "Chorus",
  lyrics: "",
  displayOrder: order,
});
const blank = (): Song => ({
...
export function Songs() {
  ...
  useEffect(() => {
    endpoints
      ...
      .catch((e) =>
        show(
          e instanceof Error ? e.message : "Failed to load cue presets",
          "error",
        ),
      );
  }, [show]);
  const sectionOptions = [
    ...new Set(cues.map((cue) => cue.label).filter(Boolean)),
  ];
  const addSection = () =>
    setEdit((current) =>
      current
        ? {
            ...
            sections: [
              ...current.sections,
              {
                ...section(current.sections.length + 1),
                sectionLabel: sectionOptions[0] || "Verse",
              },
            ],
          }
        : current,
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      const base = {
        ...
        defaultKey: edit.defaultKey.trim(),
        bpm: edit.bpm,
        chordSheet: chordEditorOpen ? edit.chordSheet.trim() : "",
      };
      const sections = edit.sections.map((s, i) => ({
        ...(s.id ? { id: s.id } : {}),
      ...
      const saved = edit.id
        ? await endpoints.updateSong({ id: edit.id, ...base, sections })
        : await endpoints.createSong({
            ...base,
            sections: sections.map(
              ({ sectionLabel, lyrics, displayOrder }) => ({
                sectionLabel,
                lyrics,
                displayOrder,
              }),
      ...
      show(e instanceof Error ? e.message : "Failed to save song", "error");
    }
  };
  const updateSection = (
    index: number,
    field: "sectionLabel" | "lyrics",
    value: string,
  ) =>
    setEdit((current) =>
      current
  ...
  return (
    <>
      <PageHeader
        title="Songs"
        description="Manage songs, lyric sections, and optional chord sheets."
        action={
          <button
            className="btn-primary"
            onClick={() => {
              const next = blank();
              next.sections[0].sectionLabel =
                sectionOptions[0] || next.sections[0].sectionLabel;
              setChordEditorOpen(false);
              setEdit(next);
            }}
          >
      ...
      <label className="relative mb-5 block max-w-xl">
        <Search className="absolute left-4 top-3.5 muted" size={18} />
        <input
      ...
      {!loading && !songs.length ? (
        <EmptyState
          title="No songs yet"
          description="Add the first song and organize its lyric sections."
        />
      ) : (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {songs.map((song) => (
            <article className="surface p-5" key={song.id}>
              <div className="flex items-start gap-3">
                <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-brand-500/10 text-brand-500">
                  <Music2 size={20} />
                </span>
                <div className="min-w-0 flex-1">
                  <h2 className="truncate font-bold">{song.title}</h2>
                  <p className="text-sm muted">{song.artist}</p>
                </div>
              </div>
              <div className="my-4 flex flex-wrap gap-2 text-xs">
                <span className="chip bg-slate-100 dark:bg-slate-900">
                  Key {song.defaultKey}
                </span>
                <span className="chip bg-slate-100 dark:bg-slate-900">
                  {song.sections?.length || 0} sections
                </span>
                {song.chordSheet?.trim() && (
              ...
              <div className="flex gap-2">
                <button
                  ...
                  onClick={() => {
                    ...
                    setChordEditorOpen(Boolean(next.chordSheet.trim()));
                    setEdit(next);
                  }}
                >
                  <Edit3 size={16} /> Edit
        ...
        {edit && (
          ...
          >
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <label>
                ...
                <input
                  className="field"
                  required
                  value={edit.defaultKey}
                  onChange={(e) =>
                    setEdit({ ...edit, defaultKey: e.target.value })
                  }
                />
              </label>
            </div>
            ...
            <div className="flex items-center justify-between">
              <h3 className="font-bold">Lyrics sections</h3>
              <button
                type="button"
            ...
            {!sectionOptions.length && (
              <p className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-500">
                Create a Cue Preset first before adding lyric sections.
              </p>
            )}
            {edit.sections.map((s, i) => (
              <div className="rounded-xl border p-3" key={`${s.id}-${i}`}>
                <div className="mb-2 flex gap-2">
                  <ModernSelect
                    className="flex-1"
                    ariaLabel="Lyrics section cue"
                    options={[...new Set([s.sectionLabel, ...sectionOptions])]
                      .filter(Boolean)
                      .map((value) => ({ value }))}
                    value={s.sectionLabel}
                    onValueChange={(value) =>
                      updateSection(i, "sectionLabel", value)
                    }
                  />
                  <button
                    type="button"
                    ...
                    onClick={() =>
                      setEdit({
                        ...edit,
                        sections: edit.sections.filter((_, x) => x !== i),
                      })
                    }
                  >
                    <X size={16} />
            ...
            <button
              ...
              disabled={!edit.sections.length || !sectionOptions.length}
            >
              Save song
            </button>
          </form>
      ...
      <ConfirmModal
        open={!!remove}
        title="Delete song?"
        description={`Delete ${remove?.title || "this song"} and all lyric sections?`}
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            show(
              e instanceof Error ? e.message : "Failed to delete song",
              "error",
            );
          }
        }}
